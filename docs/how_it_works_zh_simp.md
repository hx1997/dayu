...
低级和中级 IR 指令类似于[三地址码](https://zhuanlan.zhihu.com/p/549728632#h_549728632_5)，但函数调用可能会有多于三个参数，因此代码中将这些指令称为 `NAddressCode`，意为“N 地址码”，简称 NAC。
...
除了大体的原理，还有一些细节需要考虑。首先，要先处理内层嵌套的结构，再处理外层嵌套的结构。其次，有时会遇到**非正常区域**（improper regions），指的是不能归入任何一个预先定义的模式的区域。这种区域的一般处理方式是切断其中一些边，使其转化为正常区域，而每条切断的边所代表的跳转，需要在其源块最后插入一条 `goto` 来等价表示。蒲篮目前不处理非正常区域。

阶段 5 还进行变量分配，即把寄存器重命名为变量，以及其他一些改善可读性的小优化。

### 异常区域处理

前面讲的都是 `if-else`、`while` 这类结构的还原，`try-catch` 又该怎么处理？

你也许会想：异常也是一种跳转，直接把 throw 所在块连一条边到 handler 入口，让结构化分析一并处理就好了。问题在于，化简器并不区分"try 体"和"handler 体"——它看到的只是基本块和边。结果要么是把 try 体里的某个块和 handler 体里的块合并到一起，要么是让 handler 的块参与到某个横跨 try 边界的 `while` 循环化简里，无论哪种情况，生成的代码都是错的。

蒲篮的做法是把异常信息彻底隔离在普通 CFG 之外，另用一个专门的 pass 来处理。遇到 Panda Assembly 中的 `.catchall` 指令时：

```
.catchall try_begin, try_end, handler_begin, handler_end
```

蒲篮只是把这四个标签名存成一个 `PandasmTryCatchRegion` 对象，挂在方法上，然后继续往下走。没有任何边连到 handler 块——对 CFG 而言，异常并不存在。

等 CFG 构建完毕，`BuildCFG` 再回过头来处理这些记录，把每一个解析成 IR 层的 `ResolvedTryRegion`——里面存着两组基本块 ID，一组属于 try 体，一组属于 handler 体。具体做法是把四个边界标签映射到对应的基本块，再收集汇编文本里落在这两对标签之间的所有基本块。这两组 ID 通过每个基本块上的 `origin_block_ids` 字段在后续分析中保持可查：即便某个块被归约合并了，它的 `origin_block_ids` 仍然保存着它所吸收的原始块编号，"这个合并后的块里包含 try 体的块吗？"始终有确切答案。

此后，结构化分析照常运行，只多了一道检查：每次尝试化简某个区域之前，先确认这次化简是否会跨越 try 体与 handler 体的边界、或 try 体与外部代码的边界，若跨越则跳过。就这一道防线，便足以将两侧隔开。

结构化分析结束后，专门的 `ProcessTryCatchRegions` 接手剩余的工作：取出那两组块 ID，按它们在汇编文本里出现的先后顺序分别拼接 try 体和 handler 体的内容，外面套上 `try { ... } catch { ... }` 的文本外壳，再用新节点替换掉 CFG 里原来那些分散的块，最后将方法内所有块也按汇编文本里的先后顺序重排，让输出从上到下与原始代码的顺序一致。

这套方式能正确还原单层和嵌套的 `.catchall`。目前有两个已知限制：带类型的 `.catch` 指令（如 `.catch ExceptionType, ...`）尚未支持，解析时会静默跳过；对于包含 async/await 生成器模式的复杂方法，`try-catch` 的大结构能正确还原，但内部仍可能残留一些底层的跳转指令或标签，尚待后续优化。
...
## 附录：数据流分析
数据流分析是静态程序分析中一种重要的技术，通过对指令、基本块、函数乃至整个程序所产生的效果进行建模，数据流分析能够回答数据是如何在程序中传播的。为了简化描述，在此只讨论过程内分析，即对函数和更小的单元进行的分析。

首先回顾数据流分析中的一些常用概念。

在函数的第一条指令之前、最后一条指令之后、每两条指令之间都存在一个**程序点**，说某程序“在程序点 *p*”意思是它处于这样一种状态：*p* 之前的指令都已经执行，而之后的指令尚未执行。

变量的**定义**是指一条对该变量进行赋值/重新赋值的指令。变量的**使用**是指一次对该变量值的访问（读取但不写入）。**杀死**某定义意思是该定义所赋予的值被新的值替代，从而不再有效。

接下来，确定你的分析目标。你想获得关于函数的什么信息？比如你想知道一个定义在某程序点是否还有效，此分析称为**到达定义分析**，后文将以此为例。

然后，根据目标来对指令的效果进行建模。对于到达定义分析来说，“效果”指的是这条指令定义了哪些变量，又杀死了之前的哪些定义。

再接着，将建模推广到基本块。对每个块维护一个 `gen` 集合，记录该块中没有被杀死的新定义，以及一个 `kill` 集合，记录函数的所有定义中，该块杀死的定义。这一步做的事其实和上一步差不多，只是这一步分析的是每个基本块定义和杀死的东西。最开始两集合都是空集，随着对一条条的指令分析，逐步修改集合，对每个块都重复这一过程。

最后，一旦所有块的两个集合都计算好了，就可以对整个函数计算最终的到达定义信息。从某个块的入口处开始，先得到在该处仍有效的定义（如果是第一个基本块，入口处有效定义显然为空集，因为此时还未定义任何变量），从中删除该块的 `kill` 集合中的所有定义，然后再加入 `gen` 集合中的所有定义，就得到该块出口处有效的定义，这些定义同时也必定在该块的后继入口处有效。对后继块、后继块的后继块等等重复此过程，算法最终会收敛，此时对任何一个块，我们都知道其出口处哪些变量定义有效了。该算法的细节可在编译原理教材或者网上找到。
